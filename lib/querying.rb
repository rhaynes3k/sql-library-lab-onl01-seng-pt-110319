...
def select_character_names_and_number_of_books_they_are_in
  "SELECT char.name, COUNT(bks.title) AS bkCnt FROM Characters AS char LEFT OUTER JOIN Series AS ser ON ser.author_id = char.author_id JOIN Books AS bks ON bks.series_id = ser.id GROUP BY char.name HAVING bks_Cnt = SELECT MAX(bkCnt) FROM SELECT char.name, COUNT(bks.title) bkCnt FROM char";
end
